...
async def stream_music_visualization(websocket: WebSocket):
    ...
    try:
        ...
        base_noise_x = torch.randn(
            latent_shape,
            device=_config.diffusion.device,
            dtype=_config.diffusion.get_torch_dtype(),
        )
        torch.manual_seed(43)
        base_noise_y = torch.randn(
            latent_shape,
            device=_config.diffusion.device,
            dtype=_config.diffusion.get_torch_dtype(),
        )
        ...
        target_fps = 60  # conservative for real-time (increase with better hardware)
        frame_time = 1.0 / target_fps
        num_demo_frames = 1000  # demo length
        ...
        while frame_counter < num_demo_frames:
            ...
            angle = (
                (frame_counter / num_demo_frames) * 2 * math.pi * 3
            )  # 3 full rotations
            ...
            latent = (
                torch.cos(angle_tensor) * base_noise_x
                + torch.sin(angle_tensor) * base_noise_y
            )
